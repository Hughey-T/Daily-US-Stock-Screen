...
def verify_github_prediction(entry:dict[str,Any],repository:str,branch:str,commit_sha:str,fetch:Callable[[str],bytes])->dict[str,Any]:
 ...
 base='https://raw.githubusercontent.com';commit_url=f'{base}/{repository}/{commit_sha}/{entry["path"]}';branch_url=f'{base}/{repository}/{branch}/{entry["path"]}';index_url=f'{base}/{repository}/{commit_sha}/docs/predictions/index-v2.json';branch_index_url=f'{base}/{repository}/{branch}/docs/predictions/index-v2.json'
 commit_bytes=fetch(commit_url);branch_bytes=fetch(branch_url);index_bytes=fetch(index_url);branch_index_bytes=fetch(branch_index_url);index=json.loads(index_bytes)
 if commit_bytes!=branch_bytes:raise ValueError('pushed branch does not expose the verified commit bytes')
 if index_bytes!=branch_index_bytes:raise ValueError('pushed branch does not expose the verified index bytes')
 ...
 return {'state':'integrity_verified','repository':repository,'branch':branch,'commit_sha':commit_sha,'file_url':commit_url,'branch_url':branch_url,'sha256':actual,'index_url':index_url,'branch_index_url':branch_index_url,'index_sha256':hashlib.sha256(index_bytes).hexdigest(),'schema_version':entry['schema_version'],'source_snapshot_id':entry['source_snapshot_id'],'record_count':entry['record_count'],'created_at':entry['created_at']}
