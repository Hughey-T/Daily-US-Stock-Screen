...
import pandas as pd
from src.analysis_pipeline import process_all_rows
...
class IntegrityV2Tests(unittest.TestCase):
    ...
    def test_phase2_processes_empty_and_full_routes(self):
        rows,audit=process_all_rows([{'ticker':'A'}],[{'ticker':'B'}],'2026-01-02','a'*64)
        self.assertEqual(len(rows),2); self.assertEqual(audit['event_anomaly']['unprocessed'],0)
        _,audit=process_all_rows([],[],'2026-01-02','a'*64); self.assertEqual(audit['quiet_drift']['total'],0)
